...
String readStringEEPROM(int enderecoInicio, int maxLength)
{
  String str = "";
...
void saveParamsEEPROM()
{
  ...
  saveStringEEPROM(SSID_MAX_LENGTH + PASSWORD_MAX_LENGTH, mqtt_server, BROKER_IP_MAX_LENGTH);
...
void readParamsEEPROM()
{
  ...
  mqtt_server = readStringEEPROM(BROKER_IP_ADDRESS, BROKER_IP_MAX_LENGTH);
